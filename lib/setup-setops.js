...
async function run() {
  try {
    ...
    const loginUrl = core.getInput('setops_url');
    const loginUsername = core.getInput('setops_username');
    const loginPassword = core.getInput('setops_password');

    if ((loginUsername || loginPassword || loginUrl) && !(loginUsername && loginPassword && loginUrl)) {
      throw new Error(
        `When providing setops_username, setops_password or setops_url, all of them must be set`
      );
    }
    ...
    if (loginUsername && loginPassword && loginUrl) {
      await cli.login(loginUrl, loginUsername, loginPassword)
    }
  } catch (error) {
    core.error(error);
    throw error;
